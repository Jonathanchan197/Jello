...
const Profile = () => {
  ...
  const handleAccept = async (e, currentIndex, selectedWorkspace) => {
    ...
    const tempWorkspaceData = await db.collection('Workspaces')
      ...
      <CardGroup>
        <Card border="dark" style={{ width: "18rem" }}>
          <Card.Body>
            ...
            <Card.Text>
              some meaningful quote some meaningful quote some meaningful quote
              some meaningful quote some meaningful quote some meaningful quote
              some meaningful quote some meaningful quote some meaningful quote
              some meaningful quote some meaningful quote
            </Card.Text>
        ...
        <Card border-radius="20%" border="dark" style={{ width: "18rem" }}>
          ...
          {notifications? notifications.notifications.map((n, index) => 
          <ListGroup.Item key={index}>
            <p className="fs-4 text-capitalize">{n.workspace}</p>
            <p className="fs-8">From: ✨{n.user}✨</p>
          <div className="text-right" >
            <Accept onClick={(e)=> {handleAccept((e), {index}, n.workspace)}}> 🤝 </Accept>
            <Decline onClick={(e)=> {handleDecline((e), {index}, n.workspace)}}> ❌ </Decline>
          </div>
        </ListGroup.Item>
          ) : <></>}
          </ListGroup>
